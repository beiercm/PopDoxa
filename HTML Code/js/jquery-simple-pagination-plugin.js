	...
	*/
	...
	return this.each(function()
	{
		var container_id = '#' + $(this).attr('id'),
			...
			item_count = items.length, //made a change here in the items
		...
		{
			items.hide();
			items.slice(item_range_min, (item_range_max + 1)).show(); //made a change here to view all of the '5' items
		}
		...
		function refresh_simple_pagination(page_number)
		{
			...
			{
				var showing_x_of_x_html;
				if(item_count == item_range_max)
					showing_x_of_x_html = settings.showing_x_of_x_content + ' ' + simple_number_formatter(item_range_min + 1, 0, settings.thousands_separator) + ' to ' + simple_number_formatter((item_range_max - 1), 0, settings.thousands_separator) + ' of ' + simple_number_formatter((item_count - 1), 0, settings.thousands_separator)+ ' entries ';
				else
					showing_x_of_x_html = settings.showing_x_of_x_content + ' ' + simple_number_formatter(item_range_min + 1, 0, settings.thousands_separator) + ' to ' + simple_number_formatter((item_range_max), 0, settings.thousands_separator) + ' of ' + simple_number_formatter((item_count - 1), 0, settings.thousands_separator)+ ' entries ';
...
$.fn.simplePagination.defaults = {
	...
	//
	first_content: '&laquo;',  //e.g. '<<'
	previous_content: '&lsaquo;',  //e.g. '<'
	next_content: '&rsaquo;',  //e.g. '>'
	last_content: '&raquo;', //e.g. '>>'
